...
class Database:
    def __init__(self, db_path="data/responses.db"):
    ...
    def init_tables(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("""
                ...
                VALUES (1, 'default', 'Здравствуйте! Я занимаюсь {keywords}. Сделаю качественно и в срок. Давайте обсудим детали в личных сообщениях.')
            """)
            
            # Добавляем фильтры по умолчанию (только если таблица пустая)
            cursor = conn.execute("SELECT COUNT(*) FROM filters")
            if cursor.fetchone()[0] == 0:
                ...
                for key, value in default_filters.items():
                    conn.execute("INSERT INTO filters (key, value) VALUES (?, ?)", (key, value))
